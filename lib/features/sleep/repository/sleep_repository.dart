...
class SleepRepository {
  ...
  Future<bool> addSleep({
    ...
    final url = Uri.parse(
        'https://api.diavan-valuation.asia/api/HealthIndicator/sleep-time');
      ...
      if (response.statusCode == 200) {
        if (data["status"] == 1) {
          CherryToast.success(
              ...
              title: Text("Thời gian ngủ đã được thêm thành công!",
                  style: TextStyle(
                    color: Colors.black,
                    fontSize: 20,
                  ))).show(context);
  ...
  Future<bool> deleteSleep({
    required BuildContext context,
    required int sleepId,
  }) async {
    final url = Uri.parse(
        'https://api.diavan-valuation.asia/api/HealthIndicator/update-status/sleepTime/$sleepId');
    ...
    try {
      ...
      if (response.statusCode == 200) {
        if (data["status"] == 1) {
          CherryToast.success(
              toastDuration: Duration(seconds: 2),
              title: Text("Đã xóa chỉ số thành công!",
                  style: TextStyle(
                    color: Colors.black,
                    fontSize: 20,
                  ))).show(context);
  ...
  Future<List<Map<String, dynamic>>> getHeartRateDetail(
    ...
    final url = Uri.parse(
        'https://api.diavan-valuation.asia/api/HealthIndicator/healthIndicator/sleepTime/detail/$id');

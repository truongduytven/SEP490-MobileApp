...
class HealthMonitoringBook extends ConsumerStatefulWidget {
  final String initialTopic;
  const HealthMonitoringBook({super.key, required this.initialTopic});

  @override
  ConsumerState<HealthMonitoringBook> createState() =>
      _HealthMonitoringBookState();
}

class _HealthMonitoringBookState extends ConsumerState<HealthMonitoringBook>
  ...
  void didPopNext() {
    ...
    fetchLogBookHealthIndicator(); // Gọi lại API
  }

  Future<void> fetchLogBookHealthIndicator() async {
    setState(() {
      isLoading = true;
    });
    ...
    try {
      ...
      CherryToast.error(
        ...
        title: Text(
          "Error fetching health log book indicators: $e",
          style: TextStyle(
            color: Colors.black,
            fontSize: 16,
          ),
  ...
  List<Map<String, String>> formatApiData(List<dynamic> result) {
    ...
    return result.map((item) {
      final String tabs = item["tabs"]?.toString() ?? "";
  ...
  Widget build(BuildContext context) {
    ...
    List<Map<String, String>> filteredData = dataFromApi.where((item) {
    ...
    return Scaffold(
      ...
      body: Column(
        children: [
          ...
          Expanded(
            child: isLoading
                ? SkeletonList() // Hiển thị skeleton khi đang tải dữ liệu
                    ...
                    ? ListView(
                        children: filteredGroupedData.entries.map((entry) {
                          ...
                          return TweenAnimationBuilder(
                            tween: Tween<Offset>(
                              begin:
                                  const Offset(0, 0.8), // Start slightly below
                              end:
                                  const Offset(0, 0), // Move to normal position
                            ),
                            duration: Duration(
                                milliseconds: 1000), // Add delay per item

...
class WeightRepository {
  Future<Map<String, String>> getWeightEvaluation(
    ...
    final url = Uri.parse(
        'https://api.diavan-valuation.asia/api/HealthIndicator/healthIndicator/evaluation/bmi?weight=$weight&accountId=$accountId');
  ...
  Future<bool> addWeight({
    ...
    final url = Uri.parse(
        'https://api.diavan-valuation.asia/api/HealthIndicator/weight');
      ...
      if (response.statusCode == 200) {
        if (data["status"] == 1) {
          CherryToast.success(
              ...
              title: Text("Cân nặng đã được thêm thành công!",
                  style: TextStyle(
                    color: Colors.black,
                    fontSize: 20,
                  ))).show(context);
  ...
  Future<bool> updateWeight({
    ...
    final url = Uri.parse(
        'https://api.diavan-valuation.asia/api/HealthIndicator/update-weight/$weightId?createdBy=$createdBy');
    ...
    try {
      ...
      if (response.statusCode == 200) {
        if (data["status"] == 1) {
          CherryToast.success(
              toastDuration: Duration(seconds: 2),
              title: Text("Cân nặng đã cập nhật thành công!",
                  style: TextStyle(
                    color: Colors.black,
                    fontSize: 20,
                  ))).show(context);
  ...
  Future<bool> deleteWeight({
    ...
    final url = Uri.parse(
        'https://api.diavan-valuation.asia/api/HealthIndicator/update-status/weight/$weightId');
    ...
    try {
      ...
      if (response.statusCode == 200) {
        if (data["status"] == 1) {
          CherryToast.success(
              toastDuration: Duration(seconds: 2),
              title: Text("Đã xóa cân nặng thành công!",
                  style: TextStyle(
                    color: Colors.black,
                    fontSize: 20,
                  ))).show(context);
  ...
  Future<List<Map<String, dynamic>>> getWeightDetail(
    ...
    final url = Uri.parse(
        'https://api.diavan-valuation.asia/api/HealthIndicator/healthIndicator/weight/detail/$id');
    ...
    try {
      ...
      if (response.statusCode == 200) {
        ...
        if (data["status"] == 1) {
          final List<dynamic> weightData = data["data"]["responseList"];
